...
impl From<JsonError> for EmitError {
    fn from(e: JsonError) -> Self { EmitError::Json(e) }
}

pub trait Emit {
    fn emit(&mut self, bytes: &[u8]) -> core::result::Result<(), EmitError>;
    fn emit_byte(&mut self, b: u8) -> core::result::Result<(), EmitError> { self.emit(&[b]) }
}
...
impl<'a> SliceWriter<'a> {
    ...
    pub fn finish(self) -> &'a [u8] { &self.buf[..self.len] }
}

impl Emit for SliceWriter<'_> {
    fn emit(&mut self, bytes: &[u8]) -> core::result::Result<(), EmitError> {
        let end = self.len.checked_add(bytes.len()).ok_or(EmitError::BufferTooSmall)?;
        if end > self.buf.len() { return Err(EmitError::BufferTooSmall); }
...
pub trait HashSink { fn update(&mut self, bytes: &[u8]); }

pub struct HashWriter<'a, H: HashSink> { hasher: &'a mut H }
impl<'a, H: HashSink> HashWriter<'a, H> { pub fn new(hasher: &'a mut H) -> Self { Self { hasher } } }
...
#[derive(Clone, Copy)]
struct Entry<'a> { key: &'a [u8], value: &'a [u8] }

pub fn canonicalize_json_to_buf<'a>(json_bytes: &[u8], out: &'a mut [u8]) -> core::result::Result<&'a [u8], EmitError> {
    let mut writer = SliceWriter::new(out);
    canonicalize_to(json_bytes, &mut writer)?;
    Ok(writer.finish())
}

pub fn canonicalize_json_to_hasher<H: HashSink>(json_bytes: &[u8], hasher: &mut H) -> core::result::Result<(), EmitError> {
    let mut writer = HashWriter::new(hasher);
    canonicalize_to(json_bytes, &mut writer)
}

pub fn canonicalize_to<W: Emit>(json_bytes: &[u8], out: &mut W) -> core::result::Result<(), EmitError> {
...
fn write_value<W: Emit>(bytes: &[u8], out: &mut W) -> core::result::Result<(), EmitError> {
    ...
    match bytes.get(i).copied().ok_or(EmitError::Json(JsonError::Invalid))? {
...
fn write_object<W: Emit>(bytes: &[u8], out: &mut W) -> core::result::Result<(), EmitError> {
    let mut entries = [Entry { key: &[], value: &[] }; 16];
    let mut len = 0usize;
    let mut i = json::skip_ws(bytes, 0);
    if bytes.get(i) != Some(&b'{') { return Err(EmitError::Json(JsonError::WrongType)); }
    i += 1;
    i = json::skip_ws(bytes, i);
    if bytes.get(i) != Some(&b'}') {
        loop {
            if len >= entries.len() { return Err(EmitError::TooManyObjectFields); }
            let (ks, ke) = json::string_span(bytes, i)?;
            i = json::skip_ws(bytes, ke);
            if bytes.get(i) != Some(&b':') { return Err(EmitError::Json(JsonError::Invalid)); }
            i += 1;
            let (vs, ve) = json::value_span(bytes, i)?;
            entries[len] = Entry { key: &bytes[ks..ke], value: &bytes[vs..ve] };
            len += 1;
            i = json::skip_ws(bytes, ve);
            match bytes.get(i) {
                Some(b',') => { i += 1; i = json::skip_ws(bytes, i); }
                Some(b'}') => break,
