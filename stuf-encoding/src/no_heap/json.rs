...
fn value_span_inner(bytes: &[u8], i: usize, depth: usize) -> Result<(usize, usize)> {
    if depth > 64 { return Err(JsonError::NestingTooDeep); }
    if i >= bytes.len() { return Err(JsonError::Invalid); }
    match bytes[i] {
...
pub fn string_span(bytes: &[u8], i: usize) -> Result<(usize, usize)> {
    if bytes.get(i) != Some(&b'"') { return Err(JsonError::WrongType); }
    let mut j = i + 1;
    while j < bytes.len() {
        match bytes[j] {
            b'"' => return Ok((i, j + 1)),
            b'\\' => {
                j += 1;
                if j >= bytes.len() { return Err(JsonError::Invalid); }
                match bytes[j] {
                    b'"' | b'\\' | b'/' | b'b' | b'f' | b'n' | b'r' | b't' => j += 1,
                    b'u' => {
                        if j + 4 >= bytes.len() { return Err(JsonError::Invalid); }
                        for k in j+1..=j+4 {
                            if !bytes[k].is_ascii_hexdigit() { return Err(JsonError::Invalid); }
                        }
                        j += 5;
                    }
...
fn number_span(bytes: &[u8], mut i: usize) -> Result<(usize, usize)> {
    ...
    if i < bytes.len() && bytes[i] == b'.' {
        i += 1;
        if i >= bytes.len() || !bytes[i].is_ascii_digit() { return Err(JsonError::Invalid); }
        while i < bytes.len() && bytes[i].is_ascii_digit() { i += 1; }
    }
...
fn object_span(bytes: &[u8], mut i: usize, depth: usize) -> Result<(usize, usize)> {
    ...
    i += 1;
    i = skip_ws(bytes, i);
    if bytes.get(i) == Some(&b'}') { return Ok((start, i + 1)); }
    loop {
        let (_, ke) = string_span(bytes, i)?;
        i = skip_ws(bytes, ke);
        if bytes.get(i) != Some(&b':') { return Err(JsonError::Invalid); }
        i += 1;
        let (_, ve) = value_span_inner(bytes, i, depth + 1)?;
        i = skip_ws(bytes, ve);
        match bytes.get(i) {
            Some(b',') => { i += 1; i = skip_ws(bytes, i); }
...
pub fn parse_value(bytes: &[u8]) -> Result<Value<'_>> {
    ...
    if skip_ws(bytes, e) != bytes.len() { return Err(JsonError::Invalid); }
    Ok(Value { bytes: &bytes[s..e] })
}
...
pub fn find_object_field<'a>(object: &'a [u8], name: &str) -> Result<Value<'a>> {
    ...
    if object.get(i) != Some(&b'{') { return Err(JsonError::WrongType); }
    i += 1;
    i = skip_ws(object, i);
    if object.get(i) == Some(&b'}') { return Err(JsonError::NotFound); }
    loop {
        let (ks, ke) = string_span(object, i)?;
        let raw_key = &object[ks + 1..ke - 1];
        i = skip_ws(object, ke);
        if object.get(i) != Some(&b':') { return Err(JsonError::Invalid); }
        i += 1;
        let (vs, ve) = value_span(object, i)?;
        if key_eq(raw_key, name.as_bytes()) {
            return Ok(Value { bytes: &object[vs..ve] });
        }
        i = skip_ws(object, ve);
        match object.get(i) {
            Some(b',') => { i += 1; i = skip_ws(object, i); }
...
pub fn as_str(bytes: &[u8]) -> Result<&str> {
    ...
    if inner.contains(&b'\\') { return Err(JsonError::WrongType); }
...
pub fn as_u64(bytes: &[u8]) -> Result<u64> {
    ...
    let mut out = 0u64;
    for b in s.as_bytes() {
        if !b.is_ascii_digit() { return Err(JsonError::WrongType); }
        out = out.checked_mul(10).ok_or(JsonError::Invalid)?;
        out = out.checked_add((b - b'0') as u64).ok_or(JsonError::Invalid)?;
    }
    Ok(out)
}
...
pub fn array_items<'a, const N: usize>(array: &'a [u8], out: &mut [&'a [u8]; N]) -> Result<usize> {
    ...
    if array.get(i) != Some(&b'[') { return Err(JsonError::WrongType); }
    i += 1;
    i = skip_ws(array, i);
    if array.get(i) == Some(&b']') { return Ok(0); }
    let mut len = 0;
    loop {
        if len >= N { return Err(JsonError::TooManyItems); }
        let (s, e) = value_span(array, i)?;
        out[len] = &array[s..e];
        len += 1;
        i = skip_ws(array, e);
        match array.get(i) {
            Some(b',') => { i += 1; i = skip_ws(array, i); }
...
pub fn item_at<'a>(array: &'a [u8], index: usize) -> Result<&'a [u8]> {
    ...
    if index >= n { return Err(JsonError::NotFound); }
    Ok(tmp[index])
}
...
pub fn object_entries<'a, const N: usize>(object: &'a [u8], out: &mut [ObjectEntry<'a>; N]) -> Result<usize> {
    let mut i = skip_ws(object, 0);
    if object.get(i) != Some(&b'{') { return Err(JsonError::WrongType); }
    i += 1;
    i = skip_ws(object, i);
    if object.get(i) == Some(&b'}') { return Ok(0); }
    let mut len = 0usize;
    loop {
        if len >= N { return Err(JsonError::TooManyItems); }
        let (ks, ke) = string_span(object, i)?;
        let raw_key = &object[ks + 1..ke - 1];
        if raw_key.contains(&b'\\') { return Err(JsonError::WrongType); }
        let key = core::str::from_utf8(raw_key).map_err(|_| JsonError::Invalid)?;
        i = skip_ws(object, ke);
        if object.get(i) != Some(&b':') { return Err(JsonError::Invalid); }
        i += 1;
        let (vs, ve) = value_span(object, i)?;
        out[len] = ObjectEntry { key, raw_key, value: &object[vs..ve] };
        len += 1;
        i = skip_ws(object, ve);
        match object.get(i) {
            Some(b',') => { i += 1; i = skip_ws(object, i); }
