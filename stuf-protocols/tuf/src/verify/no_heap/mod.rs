...
use crate::{error::{Error, Result}, verify::limits::Limits};
...
enum KeyType { Ed25519, Other }
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum Scheme { Ed25519, Other }
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum RoleName { Root, Timestamp, Snapshot, Targets }
...
#[derive(Clone, Copy, Debug)]
struct SignatureView<'a> { keyid: &'a str, sig: [u8; 64] }
...
#[derive(Clone, Copy, Debug)]
struct SnapshotView<'a> { expires: u64, version: u32, targets: MetaView<'a> }
#[derive(Clone, Copy, Debug)]
...
fn check_size(bytes: &[u8], max: usize, role: &'static str) -> Result<()> {
    if bytes.len() > max {
        Err(Error::MetadataTooLarge { role, limit: max, actual: bytes.len() })
    } else { Ok(()) }
}
    ...
    C: Clock,
{
    ...
    pub fn with_limits(root_bytes: &'root [u8], transport: T, clock: C, limits: Limits) -> Result<Self> {
        ...
        Ok(Self { root, transport, clock, limits })
    }
    ...
    pub fn verify_timestamp_bytes<'ts>(self, bytes: &'ts [u8]) -> Result<TimestampChecked<'root, 'ts, T, C>> {
        ...
        Ok(TimestampChecked { root: self.root, timestamp, transport: self.transport, clock: self.clock, limits: self.limits })
    }
}
...
where
    ...
    C: Clock,
{
    pub fn verify_snapshot(self) -> Result<SnapshotChecked<'root, 'static, T, C>> { Err(Error::Transport) }

    pub fn verify_snapshot_bytes<'snap>(self, bytes: &'snap [u8]) -> Result<SnapshotChecked<'root, 'snap, T, C>> {
        ...
        if snapshot.version != self.timestamp.snapshot.version {
            return Err(Error::VersionMismatch { expected: self.timestamp.snapshot.version, received: snapshot.version });
        }
        check_expiry(snapshot.expires, &self.clock)?;
        Ok(SnapshotChecked { root: self.root, snapshot, transport: self.transport, clock: self.clock, limits: self.limits })
    }
}
...
where
    ...
    C: Clock,
{
    pub fn verify_targets(self) -> Result<TargetsChecked<'root, 'snap, 'static, T, C>> { Err(Error::Transport) }

    pub fn verify_targets_bytes<'targets>(self, bytes: &'targets [u8]) -> Result<TargetsChecked<'root, 'snap, 'targets, T, C>> {
        ...
        if targets.version != self.snapshot.targets.version {
            return Err(Error::VersionMismatch { expected: self.snapshot.targets.version, received: targets.version });
        }
        check_expiry(targets.expires, &self.clock)?;
        Ok(TargetsChecked { root: self.root, snapshot: self.snapshot, targets, transport: self.transport, clock: self.clock, limits: self.limits })
    }
}
...
where
    ...
    C: Clock,
{
    pub fn verify_target(&self, _name: &str) -> Result<Verified<TargetView<'targets>>> { Err(Error::Transport) }

    pub fn verify_target_bytes(&self, name: &str, bytes: &[u8]) -> Result<Verified<TargetView<'targets>>> {
        let t = self.find_target(name).ok_or(Error::TargetNotFound)?;
        if bytes.len() as u64 != t.length {
            return Err(Error::TargetLengthMismatch { expected: t.length, actual: bytes.len() as u64 });
        }
...
fn signed_payload(bytes: &[u8]) -> Result<&[u8]> { enc::field(bytes, "signed").map_err(|_| Error::Deserialize) }
fn signatures(bytes: &[u8]) -> Result<&[u8]> { enc::field(bytes, "signatures").map_err(|_| Error::Deserialize) }

fn check_expiry<C: Clock>(expires: u64, clock: &C) -> Result<()> {
    if clock.now_secs() > expires { Err(Error::Expired) } else { Ok(()) }
}
...
fn parse_root<'a>(signed: &'a [u8], limits: &Limits) -> Result<RootView<'a>> {
    ...
    let mut root = RootView { expires, version, keys: [None; MAX_KEYS], keys_len: 0, roles: [None; 4] };
    ...
    if n > limits.max_keys { return Err(Error::TooManyKeys { limit: limits.max_keys, actual: n }); }
...
fn parse_key<'a>(keyid: &'a str, value: &'a [u8]) -> Result<KeyView<'a>> {
    let keytype = match get_str(value, "keytype")? { "ed25519" => KeyType::Ed25519, _ => KeyType::Other };
    let scheme = match get_str(value, "scheme")? { "ed25519" => Scheme::Ed25519, _ => Scheme::Other };
    ...
    Ok(KeyView { keyid, keytype, scheme, public })
}
...
fn parse_role_keys<'a>(_role: RoleName, value: &'a [u8]) -> Result<RoleKeysView<'a>> {
    ...
    let len = enc::array_items(keyids_array, &mut items).map_err(|_| Error::TooManyKeys { limit: MAX_ROLE_KEYIDS, actual: MAX_ROLE_KEYIDS + 1 })?;
    let mut keyids = [""; MAX_ROLE_KEYIDS];
    for i in 0..len { keyids[i] = enc::as_str(items[i]).map_err(|_| Error::Deserialize)?; }
    Ok(RoleKeysView { keyids, keyids_len: len, threshold })
}
...
fn parse_timestamp<'a>(signed: &'a [u8]) -> Result<TimestampView<'a>> {
    ...
    Ok(TimestampView { expires, version, snapshot: parse_meta(snap)? })
}
...
fn parse_snapshot<'a>(signed: &'a [u8]) -> Result<SnapshotView<'a>> {
    ...
    Ok(SnapshotView { expires, version, targets: parse_meta(targets)? })
}

fn parse_meta<'a>(bytes: &'a [u8]) -> Result<MetaView<'a>> {
    let version = get_u64(bytes, "version")? as u32;
    let length = match enc::field(bytes, "length") { Ok(v) => Some(enc::as_u64(v).map_err(|_| Error::Deserialize)?), Err(_) => None };
    let hashes = enc::field(bytes, "hashes").ok();
    let sha256 = match hashes { Some(h) => enc::field(h, "sha256").ok().map(|v| enc::as_str(v).map_err(|_| Error::Deserialize)).transpose()?, None => None };
    Ok(MetaView { version, length, sha256 })
}
...
fn parse_targets<'a>(signed: &'a [u8], limits: &Limits) -> Result<TargetsView<'a>> {
    ...
    if n > limits.max_targets_entries { return Err(Error::TooManyTargets { limit: limits.max_targets_entries, actual: n }); }
    let mut out = TargetsView { expires, version, targets: [None; MAX_TARGETS], targets_len: 0 };
    for e in entries.iter().take(n) {
        ...
        out.targets[out.targets_len] = Some(TargetView { name: e.key, length, sha256 });
        out.targets_len += 1;
    }
    Ok(out)
}

fn empty_entry<'a>() -> enc::ObjectEntry<'a> { enc::ObjectEntry { key: "", raw_key: &[], value: &[] } }

fn get_str<'a>(obj: &'a [u8], name: &str) -> Result<&'a str> {
    enc::field(obj, name).and_then(enc::as_str).map_err(|_| Error::Deserialize)
}
fn get_u64(obj: &[u8], name: &str) -> Result<u64> {
    enc::field(obj, name).and_then(enc::as_u64).map_err(|_| Error::Deserialize)
}

fn role_index(role: RoleName) -> usize {
    match role { RoleName::Root => 0, RoleName::Timestamp => 1, RoleName::Snapshot => 2, RoleName::Targets => 3 }
}
...
fn find_key<'a>(root: &RootView<'a>, keyid: &str) -> Option<KeyView<'a>> {
    for i in 0..root.keys_len {
        if let Some(k) = root.keys[i] {
            if k.keyid == keyid { return Some(k); }
        }
    }
    None
}

fn parse_signatures<'a, const N: usize>(array: &'a [u8], out: &mut [Option<SignatureView<'a>>; N]) -> Result<usize> {
    let mut items = [&[][..]; MAX_SIGNATURES];
    let len = enc::array_items(array, &mut items).map_err(|_| Error::TooManySignatures { limit: N, actual: N + 1 })?;
    if len > N { return Err(Error::TooManySignatures { limit: N, actual: len }); }
    for i in 0..len {
...
fn verify_role_signatures(signed_payload: &[u8], signatures_array: &[u8], root: &RootView<'_>, role: RoleName, limits: &Limits) -> Result<()> {
    ...
    if sig_len > limits.max_signatures { return Err(Error::TooManySignatures { limit: limits.max_signatures, actual: sig_len }); }

    let mut canonical = [0u8; MAX_CANONICAL_BYTES];
    let canonical = enc::canonicalize_json_to_buf(signed_payload, &mut canonical).map_err(|_| Error::Encoding)?;

    let mut counted = [""; MAX_SIGNATURES];
    let mut counted_len = 0usize;
    let mut valid = 0u32;
    for sig in signatures.iter().take(sig_len).flatten() {
        if !role_contains_keyid(role_keys, sig.keyid) { continue; }
        if counted[..counted_len].iter().any(|id| *id == sig.keyid) { continue; }
        if let Some(key) = find_key(root, sig.keyid) {
            if key.keytype != KeyType::Ed25519 || key.scheme != Scheme::Ed25519 { continue; }
    ...
    if valid >= role_keys.threshold { Ok(()) } else { Err(Error::ThresholdNotMet { threshold: role_keys.threshold, valid }) }
}
...
fn verify_metadata_ref(bytes: &[u8], meta: MetaView<'_>) -> Result<()> {
    if let Some(expected) = meta.length {
        ...
        if actual != expected { return Err(Error::MetadataLengthMismatch { expected, actual }); }
    }
...
fn verify_sha256_hex(bytes: &[u8], expected_hex: &str) -> Result<()> {
    if expected_hex.len() != 64 { return Err(Error::InvalidHashLength { expected: 64, actual: expected_hex.len() }); }
    if !expected_hex.as_bytes().iter().all(|b| b.is_ascii_hexdigit()) { return Err(Error::InvalidHashEncoding); }
    let actual = stuf_env::crypto::sha256(bytes);
    let mut expected = [0u8; 32];
    hex_to_bytes(expected_hex, &mut expected).map_err(|_| Error::InvalidHashEncoding)?;
    if actual == expected { Ok(()) } else { Err(Error::TargetHashMismatch) }
}

#[cfg(not(feature = "hash-sha256"))]
fn verify_sha256_hex(_bytes: &[u8], _expected_hex: &str) -> Result<()> { Err(Error::NoHashAlgorithm) }

fn hex_to_bytes(hex: &str, out: &mut [u8]) -> core::result::Result<(), ()> {
    if hex.len() != out.len() * 2 { return Err(()); }
